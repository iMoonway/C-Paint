...
- [x] 支持读取设备管理器中所有的端口(*Version 0.3.0*)
- [x] 支持刷新端口列表(*Version 0.3.0*)
- [x] 支持选择波特率(*Version 0.3.0*)
- [x] 支持串口列表刷新(*Version 0.3.0*)
- [x] 串口发送功能(*Version 0.3.0*)

## 程序截图

| 手势录制 | 端口选择(*Version 0.3.0*) |
|----------|----------|
...
串口设置的只能够单字节发送，所以8位的数据需要分为4次发送(*单次发送一字节，一字节包含2位数据*)。

同样以`0208517d`为例，

| 发送数据顺序 | 02 | 08 | 51 | 7d |
|--------------|----|----|----|----|
| 接收端可能收到 | 字符串: **0x02** <br> 10进制: **2** | 字符串: **0x08** <br> 10进制: **8** | 字符串: **0x51** <br> 10进制: **81** | 字符串: **0x7d** <br> 10进制: **125** |

所以，收到的数据请按照该[处理方法](#坐标的获取)。
